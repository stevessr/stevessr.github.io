---
title: "黑豹x2刷机"
date: 2025-03-10T19:40:38+08:00
draft: false
author: "stevessr"
keywords: ["黑豹x2", "刷机", "Armbian", "Linux", "服务器", "家庭服务器", "旁路由", "软路由", "折腾", "RKDevTool", "Amlogic", "S9xxx"]
cover: ""
summary: "本文记录了在黑豹x2电视盒子上刷入Armbian系统的详细步骤，包括固件下载、刷机工具使用、网络配置、常用软件安装以及一些进阶操作，例如挂载、代理、NPU环境配置、各种常用服务的搭建，如Alist, webssh, LibreChat, SearXNG, n8n, Anything-LLM, OpenWebUI等。旨在打造一个功能丰富的家庭服务器或旁路由环境。"
type: "yaml"
tags: ["黑豹x2", "Armbian", "刷机教程", "Linux", "家庭服务器", "旁路由", "Amlogic", "RKDevTool", "docker", "alist", "webssh","LibreChat", "SearXNG", "n8n", "Anything-LLM","OpenWebUI","NPU", "RKNN", "LLM", "大模型","node", "rust", "python"]
categories: ["服务器", "家庭网络", "Linux", "折腾", "嵌入式"]
---
...
从[ophub/amlogic-s9xxx-armbian: Support for Armbian in Amlogic, Rockchip and Allwinner boxes. Support a311d, s922x, s905x3, s905x2, s912, s905d, s905x, s905w, s905, s905l, rk3588, rk3568, rk3399, rk3328, h6, etc.](https://github.com/ophub/amlogic-s9xxx-armbian)下载对应固件

从某处下载RKDevTool_Release_v2.84
[RKDevTool \| Radxa Docs](https://docs.radxa.com/zero/zero3/low-level-dev/rkdevtool) 这里可以搞到

怼着reset，usb连接电脑（不要接其他电源）
...
``` shell
wget https://github.com/danny-avila/LibreChat/archive/refs/heads/main.zip
unzip main.zip 
rm main.zip
cd LibreChat-main
cp .env.example .env
sudo nvim .env
# 自己编辑一下
# mongodb+srv://stevessr007:MyArk50Wf05vY9ak@cluster0.yqozs.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0
```
...
默认会把usb设备挂载到`/media/user`目录下

可以通过修改/etc/udevil/udevil.conf
\# n8n来☝️
...
``` shell
pip install webssh
pm2 start wssh -- --address='0.0.0.0' --port=8000
pm2 stop wssh
```
...
``` shell
export ENV=prod
export WEBUI_NAME="steve's webui"
export CUSTOM_NAME="steve's webui"
export WEBUI_URL="https://any.aaca.eu.org"
export PORT=8088
export ENABLE_CHANNELS=TRUE
export DEFAULT_LOCALE="zh_cn"
export AIOHTTP_CLIENT_TIMEOUT_OPENAI_MODEL_LIST=10
export DATA_DIR="/mnt/sdcard/opwebui"
export DATABASE_URL="postgresql://opwebui:opwebui@localhost/opwebui"
...
HF_ENDPOINT=https://hf-mirror.com ENV=prod WEBUI_NAME="steve's webui" CUSTOM_NAME="steve's webui" WEBUI_URL="https://any.aaca.eu.org" PORT=8088 ENABLE_CHANNELS=TRUE DEFAULT_LOCALE="zh_cn" AIOHTTP_CLIENT_TIMEOUT_OPENAI_MODEL_LIST=10 DATA_DIR="/mnt/sdcard/opwebui" DATABASE_URL="postgresql://opwebui:opwebui@localhost/opwebui" ENABLE_WEBSOCKET_SUPPORT=TRUE WEBSOCKET_MANAGER="REDIS" WEBSOCKET_REDIS_URL="redis://localhost:6379/0" REDIS_URL="redis://localhost:6379/0" open-webui serve --port 8088
...
# 配合[Calcium-Ion/new-api:](https://github.com/Calcium-Ion/new-api)

``` shell
wget https://github.com/Calcium-Ion/new-api/releases/download/v0.4.8.9.1/one-api-arm64
```

``` shell
# sudo chmod +x one-api-arm64
# sudo nano new.sh
export STREAMING_TIMEOUT=6000
export REDIS_CONN_STRING="redis://localhost:6379/1"
export SQL_DSN="postgres://newapi:newapi@localhost/newapi"
export MEMORY_CACHE_ENABLED=true
export PORT=8081

STREAMING_TIMEOUT=6000 REDIS_CONN_STRING="redis://localhost:6379/1" SQL_DSN="postgres://newapi:newapi@localhost/newapi" MEMORY_CACHE_ENABLED=true PORT=8081 ./one-api-arm64
```
...
获取代码：rknn
下载 RKNPU2_SDK/2.3.0/release里面的rknn-toolkit2-v2.3.0-2024-11-08.tgz
解压里面的librknnrt.so & rknn_server 到 /usr/lib
...
export MEMOS_DSN=postgresql://memos:memos@localhost:5432/memos?sslmode=disable
export MEMOS_MODE=prod
esport MEMOS_PORT=5230
export MEMOS_DATA=/mnt/sdcard/memos

MEMOS_DRIVER=postgres MEMOS_DSN=postgresql://memos:memos@localhost:5432/memos?sslmode=disable MEMOS_MODE=prod MEMOS_PORT=5230 MEMOS_DATA=/mnt/sdcard/memos ./memos --mode prod
```
